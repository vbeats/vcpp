...
@ProtocolCmd(value = FrameType.CHARGING_MOTOR_STOP, protocols = {V15, V16, V17})
public class YkcV15ChargingMotorStopUpCmd extends YkcCmdExecutor { // 充电阶段充电机中止
	@Override
	public void execute(Channel channel, String versionLabel, YkcMsgData data) {
		...
		// 充电机中止充电原因
		String motorStopReason = YkcExceptionUtil.parseMotorStopReason(body.readByte());

		// 充电机中止充电故障原因
		byte[] faultBytes = new byte[2];
		body.readBytes(faultBytes);
		String motorFaultReason = YkcExceptionUtil.parseMotorFaultReason(faultBytes);

		// 充电机中止充电错误原因
		String motorErrorReason = YkcExceptionUtil.parseMotorErrorReason(body.readByte());


		log.info("【云快充】{} ⬆️ 充电阶段充电机中止 - 交易流水号: {} 桩号: {} 枪号: {} 中止原因: {} 故障原因: {} 错误原因: {}",
			versionLabel, tradeNo, deviceId, gunNo, motorStopReason, motorFaultReason, motorErrorReason);
